...
public class PaymentsModel {
    ...
    private Double m_dAdminTotal;
    private List<PaymentsLine> m_ladminpayments;
    ...
    public static PaymentsModel emptyInstance() {
        ...
        p.m_dAdminTotal = 0.0;
        p.m_ladminpayments = new ArrayList<PaymentsLine>();
    ...
    public static PaymentsModel loadInstance(AppView app) throws BasicException {
        ...
        List categorys = new StaticSentence<String, CategorySalesLine>(app.getSession(), "SELECT a.NAME, sum(c.UNITS), sum(c.UNITS * (c.PRICE + (c.PRICE * d.RATE))) FROM categories as a LEFT JOIN products as b on a.id = b.CATEGORY LEFT JOIN ticketlines as c on b.id = c.PRODUCT LEFT JOIN taxes as d on c.TAXID = d.ID LEFT JOIN receipts as e on c.TICKET = e.ID WHERE e.MONEY = ? GROUP BY a.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass<CategorySalesLine>(CategorySalesLine.class)).list((Object)app.getActiveCashIndex());
        p.m_lcategorysales = categorys == null ? new ArrayList<CategorySalesLine>() : categorys;
        Object[] valtickets = (Object[])new StaticSentence<String, Object[]>(app.getSession(), "SELECT COUNT(*), SUM(p.TOTAL) FROM payments p JOIN receipts r ON p.RECEIPT = r.ID LEFT JOIN payment_categories pc ON p.CATEGORY = pc.ID WHERE r.MONEY = ? AND (pc.IS_GLOBAL IS NULL OR pc.IS_GLOBAL = b'0')", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.INT, Datas.DOUBLE})).find((Object)app.getActiveCashIndex());
        if (valtickets == null) {
            p.m_iPayments = 0;
            p.m_dPaymentsTotal = 0.0;
        } else {
            p.m_iPayments = (Integer)valtickets[0];
            p.m_dPaymentsTotal = (Double)valtickets[1] == null ? 0.0 : (Double)valtickets[1];
        }
        List l = new StaticSentence<String, PaymentsLine>(app.getSession(), "SELECT p.PAYMENT, SUM(p.TOTAL), p.NOTES FROM payments p JOIN receipts r ON p.RECEIPT = r.ID LEFT JOIN payment_categories pc ON p.CATEGORY = pc.ID WHERE r.MONEY = ? AND (pc.IS_GLOBAL IS NULL OR pc.IS_GLOBAL = b'0') GROUP BY p.PAYMENT, p.NOTES", SerializerWriteString.INSTANCE, new SerializerReadClass<PaymentsLine>(PaymentsLine.class)).list((Object)app.getActiveCashIndex());
        p.m_lpayments = l == null ? new ArrayList<PaymentsLine>() : l;
        Object[] valadmin = (Object[])new StaticSentence<String, Object[]>(app.getSession(), "SELECT SUM(p.TOTAL) FROM payments p JOIN receipts r ON p.RECEIPT = r.ID LEFT JOIN payment_categories pc ON p.CATEGORY = pc.ID WHERE r.MONEY = ? AND pc.IS_GLOBAL = b'1'", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.DOUBLE})).find((Object)app.getActiveCashIndex());
        p.m_dAdminTotal = valadmin == null || valadmin[0] == null ? 0.0 : (Double)valadmin[0];
        List la = new StaticSentence<String, PaymentsLine>(app.getSession(), "SELECT p.PAYMENT, SUM(p.TOTAL), p.NOTES FROM payments p JOIN receipts r ON p.RECEIPT = r.ID LEFT JOIN payment_categories pc ON p.CATEGORY = pc.ID WHERE r.MONEY = ? AND pc.IS_GLOBAL = b'1' GROUP BY p.PAYMENT, p.NOTES", SerializerWriteString.INSTANCE, new SerializerReadClass<PaymentsLine>(PaymentsLine.class)).list((Object)app.getActiveCashIndex());
        p.m_ladminpayments = la == null ? new ArrayList<PaymentsLine>() : la;
        Object[] recsales = (Object[])new StaticSentence<String, Object[]>(app.getSession(), "SELECT COUNT(DISTINCT receipts.ID), SUM(ticketlines.UNITS * ticketlines.PRICE) FROM receipts, ticketlines WHERE receipts.ID = ticketlines.TICKET AND receipts.MONEY = ?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.INT, Datas.DOUBLE})).find((Object)app.getActiveCashIndex());
        if (recsales == null) {
            p.m_iSales = null;
            p.m_dSalesBase = null;
        } else {
        ...
        Object[] rectaxes = (Object[])new StaticSentence<String, Object[]>(app.getSession(), "SELECT SUM(taxlines.AMOUNT), SUM(taxlines.BASE) FROM receipts, taxlines WHERE receipts.ID = taxlines.RECEIPT AND receipts.MONEY = ?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{Datas.DOUBLE, Datas.DOUBLE})).find((Object)app.getActiveCashIndex());
        if (rectaxes == null) {
            p.m_dSalesTaxes = null;
            p.m_dSalesTaxNet = null;
        } else {
        ...
        List removedLines = new StaticSentence<String, RemovedProductLines>(app.getSession(), "SELECT lineremoved.NAME, lineremoved.TICKETID, lineremoved.PRODUCTNAME, SUM(lineremoved.UNITS) AS TOTAL_UNITS  FROM lineremoved WHERE lineremoved.REMOVEDDATE > ? GROUP BY lineremoved.NAME, lineremoved.TICKETID, lineremoved.PRODUCTNAME", SerializerWriteString.INSTANCE, new SerializerReadClass<RemovedProductLines>(RemovedProductLines.class)).list((Object)startDateFormatted);
        p.m_lremovedlines = removedLines == null ? new ArrayList<RemovedProductLines>() : removedLines;
        List drawerOpenedLines = new StaticSentence<String, DrawerOpenedLines>(app.getSession(), "SELECT OPENDATE, NAME, TICKETID  FROM draweropened WHERE TICKETID = 'No Sale' AND OPENDATE > ? GROUP BY NAME, OPENDATE, TICKETID", SerializerWriteString.INSTANCE, new SerializerReadClass<DrawerOpenedLines>(DrawerOpenedLines.class)).list((Object)startDateFormatted);
    ...
    public double getSalesTotal() {
        return (this.m_dSalesBase == null ? 0.0 : this.m_dSalesBase) + (this.m_dSalesTaxes == null ? 0.0 : this.m_dSalesTaxes);
    }
    ...
    public double getAdminTotal() {
        return this.m_dAdminTotal;
    }

    public String printAdminTotal() {
        return Formats.CURRENCY.formatValue(this.m_dAdminTotal);
    }

    public List<PaymentsLine> getAdminPaymentLines() {
        return this.m_ladminpayments;
    }
    ...
    public AbstractTableModel getPaymentsModel() {
        return new AbstractTableModel(){
            ...
            public Object getValueAt(int row, int column) {
                PaymentsLine l = (PaymentsLine)PaymentsModel.this.m_lpayments.get(row);
                switch (column) {
                    case 0: {
                        return l.getType();
                    }
    ...
    public AbstractTableModel getSalesModel() {
        return new AbstractTableModel(){
            ...
            @Override
            public Object getValueAt(int row, int column) {
                SalesLine l = (SalesLine)PaymentsModel.this.m_lsales.get(row);
                switch (column) {
                    case 0: {
                        return l.getTaxName();
                    }
