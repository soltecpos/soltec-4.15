...
extends JDialog {
    ...
    private JProductAttEdit2(Frame parent, boolean modal) {
        super(parent, modal);
    }

    private JProductAttEdit2(Dialog parent, boolean modal) {
        super(parent, modal);
    }
    ...
    private void init(Session s) {
        ...
        this.attsetSent = new PreparedSentence<String, AttributeSetInfo>(s, "SELECT ID, NAME FROM attributeset WHERE ID = ?", SerializerWriteString.INSTANCE, dr -> new AttributeSetInfo(dr.getString(1), dr.getString(2)));
        ...
        this.attinstSent = new PreparedSentence<String, AttributeInstInfo>(s, "SELECT A.ID, A.NAME, " + s.DB.CHAR_NULL() + ", " + s.DB.CHAR_NULL() + " FROM attributeuse AU JOIN attribute A ON AU.ATTRIBUTE_ID = A.ID WHERE AU.ATTRIBUTESET_ID = ? ORDER BY AU.LINENO", SerializerWriteString.INSTANCE, dr -> new AttributeInstInfo(dr.getString(1), dr.getString(2), dr.getString(3), dr.getString(4)));
        this.attinstSent2 = new PreparedSentence<Object[], AttributeInstInfo>(s, "SELECT A.ID, A.NAME, AI.ID, AI.VALUE FROM attributeuse AU JOIN attribute A ON AU.ATTRIBUTE_ID = A.ID LEFT OUTER JOIN attributeinstance AI ON AI.ATTRIBUTE_ID = A.ID WHERE AU.ATTRIBUTESET_ID = ? AND AI.ATTRIBUTESETINSTANCE_ID = ?ORDER BY AU.LINENO", new SerializerWriteBasic(Datas.STRING, Datas.STRING), dr -> new AttributeInstInfo(dr.getString(1), dr.getString(2), dr.getString(3), dr.getString(4)));
    ...
    public static JProductAttEdit2 getAttributesEditor(Component parent, Session s) {
        ...
        JProductAttEdit2 myMsg = window instanceof Frame ? new JProductAttEdit2((Frame)window, true) : new JProductAttEdit2((Dialog)window, true);
        myMsg.init(s);
    ...
    private void initComponents() {
        ...
        this.m_jButtonCancel.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent evt) {
                JProductAttEdit2.this.m_jButtonCancelActionPerformed(evt);
            }
        });
        ...
        this.m_jButtonOK.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent evt) {
                JProductAttEdit2.this.m_jButtonOKActionPerformed(evt);
            }
        });
        ...
        this.setSize(new Dimension(656, 388));
    ...
    private void m_jButtonOKActionPerformed(ActionEvent evt) {
        ...
        this.itemslist.stream().map(item -> item.getValue()).filter(value -> value != null && value.length() > 0).forEach(value -> {
        ...
        if (description.length() == 0) {
            ...
            if (id == null ? String.valueOf(description) != null : !id.equals(String.valueOf(description))) {
                ...
                try {
                    ...
                    for (JProductAttEditI item2 : this.itemslist) {
                        this.attinstSave.exec(new Object[]{UUID.randomUUID().toString(), id, item2.getAttribute(), item2.getValue()});
                    }
                }
                catch (BasicException ex) {
                    return;
