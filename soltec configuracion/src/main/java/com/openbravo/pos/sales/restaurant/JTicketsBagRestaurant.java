...
extends JPanel {
    ...
    public JTicketsBagRestaurant(AppView app, JTicketsBagRestaurantMap restaurant) {
        ...
        this.m_dlReceipts = (DataLogicReceipts)this.m_App.getBean("com.openbravo.pos.sales.DataLogicReceipts");
        ...
        this.m_MoveTable.setVisible(this.m_App.getAppUserView().getUser().hasPermission("sales.MoveTable"));
    }
    ...
    public void activate() {
        ...
        this.m_MoveTable.setVisible(true);
    }
    ...
    private void initComponents() {
        ...
        // Aumentar dimensiones del panel para caber botones
        this.setMinimumSize(new Dimension(230, 95));
        this.setPreferredSize(new Dimension(230, 95));
        ...
        this.m_DelTicket.setPreferredSize(new Dimension(75, 85));
        this.m_DelTicket.setMinimumSize(new Dimension(75, 85));
        this.m_DelTicket.setMaximumSize(new Dimension(75, 85));
        ...
        this.m_TablePlan.setPreferredSize(new Dimension(75, 85));
        this.m_TablePlan.setMinimumSize(new Dimension(75, 85));
        this.m_TablePlan.setMaximumSize(new Dimension(75, 85));
        ...
        this.m_MoveTable.setPreferredSize(new Dimension(75, 85));
        this.m_MoveTable.setMinimumSize(new Dimension(75, 85));
        this.m_MoveTable.setMaximumSize(new Dimension(75, 85));
    ...
    private void j_btnKitchenActionPerformed(ActionEvent evt) {
        ...
        try {
            ...
            // FORCE all items to be marked as sent visually, overriding strict script routing checks.
            for (int lineIdx = 0; lineIdx < this.ticket.getLinesCount(); lineIdx++) {
                 TicketLineInfo l = this.ticket.getLine(lineIdx);
                 l.setProperty("sendstatus", "Yes");
            }
            
            // Re-bind the ticket to the UI to reflect new properties
            this.m_restaurant.reloadActiveTicket();
            
            // PERSISTENCE FIX: Save the ticket back to the sharedtickets table so sendstatus is remembered
            if (this.ticket.getUser() == null) {
                this.ticket.setUser(new UserInfo(this.m_App.getAppUserView().getUser().getId(), this.m_App.getAppUserView().getUser().getName()));
            }
            System.out.println("cocina: Updating shared ticket in DB for table " + this.m_restaurant.getTable());
            this.m_dlReceipts.updateSharedTicket(this.m_restaurant.getTable(), this.ticket, this.ticket.getPickupId());
            
            // VERIFICATION READBACK
            TicketInfo verify = this.m_dlReceipts.getSharedTicket(this.m_restaurant.getTable());
            if (verify != null) {
                System.out.println("cocina: Verification - Ticket lines=" + verify.getLinesCount());
                for (int v=0; v < verify.getLinesCount(); v++) {
                   System.out.println("cocina: Verification - Line " + v + " status=" + verify.getLine(v).getProperty("sendstatus"));
                }
            }
            
            // Repaint the active ticket to reflect changes like the orange background.
            this.m_restaurant.reloadActiveTicket();
